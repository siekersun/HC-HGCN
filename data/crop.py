...
def crop(file_fold):
    ...
    for folder in file_fold.glob('**/*'):
        if folder.is_dir():
            ...
            #病人
            patient, Spacing = load_scan(folder)
            patient_pixels = get_pixels_hu(patient)

            #重采样
            # resampled_image, spacing = resample(patient_pixels, patient, [1, 1, 1])

            patient_image = sitk.GetImageFromArray(patient_pixels)

            resampled_image = transform(image = patient_image, newSpacing = [1, 1, 1],  resamplemethod=sitk.sitkBSpline, Spacing = Spacing, is_label = False)
            ...
            # 对每个标签进行裁剪和保存
            for label in labels:
                boundary = get_boundary_for_label(img_data, label, [1,1,1], Spacing)
